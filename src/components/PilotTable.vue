    ...
    const sorted = [...props.pilots].sort((a, b) => {
        ...
        switch (sortKey.value) {
            ...
            case 'pilot':
                ...
                break
            case 'corporation':
                comparison = (a.character.corporation_name || '').localeCompare(
                    b.character.corporation_name || ''
                )
                break
            case 'alliance':
                comparison = (a.character.alliance_name || '').localeCompare(
                    b.character.alliance_name || ''
                )
                break
            case 'kills':
                comparison =
                    (a.zkill?.ships_destroyed || 0) -
                    (b.zkill?.ships_destroyed || 0)
                break
            case 'deaths':
                comparison =
                    (a.zkill?.ships_lost || 0) - (b.zkill?.ships_lost || 0)
                break
            case 'isk':
                comparison =
            ...
            case 'danger':
                comparison =
                    (a.zkill?.danger_ratio || 0) - (b.zkill?.danger_ratio || 0)
                break
            case 'ppk': {
                const ppkA = a.zkill
                    ? a.zkill.ships_destroyed > 0
                        ? a.zkill.points_destroyed / a.zkill.ships_destroyed
                        : 0
                    : 0
                const ppkB = b.zkill
                    ? b.zkill.ships_destroyed > 0
                        ? b.zkill.points_destroyed / b.zkill.ships_destroyed
                        : 0
                    : 0
...
<template>
    <main class="flex-1 flex flex-col overflow-hidden">
        <div
            class="grid grid-cols-[54px_minmax(120px,1fr)_minmax(100px,1.2fr)_minmax(80px,1fr)_160px_100px_90px_50px_50px] gap-2 px-4 py-2 bg-eve-bg-2 border-b border-eve-border text-[10px] font-semibold tracking-wider text-eve-text-3 uppercase shrink-0"
        >
            <SortableHeader
                label="Threat"
                sort-key="threat"
