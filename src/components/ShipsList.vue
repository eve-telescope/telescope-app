...
function openShipUrl(shipTypeId: number, type: 'kills' | 'losses' | 'all'): void {
  const suffix = type === 'all' ? '' : `${type}/`;
  openUrl(`https://zkillboard.com/character/${props.characterId}/ship/${shipTypeId}/${suffix}`);
}
</script>
...
<template>
  <div v-if="ships.length" class="space-y-1">
    ...
    >
      ...
      <span class="font-mono text-eve-green text-[11px]">{{ ship.kills }}</span>
      
      <!-- Hover links -->
      <div class="absolute right-0 opacity-0 group-hover:opacity-100 transition-opacity flex gap-0.5">
        <button
